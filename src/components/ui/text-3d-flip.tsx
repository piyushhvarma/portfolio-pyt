...
const Text3DFlip = ({
  ...
  // Automatically trigger animation when children change
  useEffect(() => {
    // Only trigger if we are mounted and not the initial render hook?
    // Let's just run it whenever `text` changes!
    const timeoutId = setTimeout(() => {
      handleHoverStart()
    }, 50)
    return () => clearTimeout(timeoutId)
  }, [text, handleHoverStart])
  ...
  return (
    ...
    >
      ...
      {characters.map((wordObj, wordIndex) => (
        <span key={wordIndex} className="inline-flex [transform-style:preserve-3d]">
...
const CharBox = memo(
  ({
    ...
    >
      <span
        className={cn("relative h-[1lh] [backface-visibility:hidden]", textClassName)}
      ...
      <span
        className={cn(
          "absolute top-0 left-0 h-[1lh] [backface-visibility:hidden]",
          flipTextClassName
        )}
